import { Cl as RGBFormat, Es as MathUtils, Hl as RenderTarget, Ht as NodeMaterial, ar as TempNode, dd as Vector2, jn as RendererUtils, nd as UnsignedByteType, rd as UnsignedInt101111Type, yn as QuadMesh, zl as RedFormat } from "./three.webgpu-CajSZNVS.js";
import { Break, Fn, HALF_PI, If, Loop, NodeUpdateType, PI, abs, acos, bool, ceil, clamp, convertToTexture, cos, countOneBits, cross, div, dot, float, fract, getNormalFromDepth, getViewPosition, interleavedGradientNoise, logarithmicDepthToViewZ, luminance, max, mul, normalize, outputStruct, passTexture, pow, property, rand, reference, screenCoordinate, shiftRight, sign, sin, sqrt, sub, uint, uniform, uv, vec2, vec3, vec4, viewZToPerspectiveDepth } from "./three_tsl.js";
...
*/
var SSGINode = class extends TempNode {
	static get type() {
		return "SSGINode";
	}
	/**
	...
	*/
	setSize(width, height) {
		...
		this._halfProjScale.value = height / (Math.tan(this._camera.fov * MathUtils.DEG2RAD * .5) * 2) * .5;
	}
	/**
	...
	*/
	setup(builder) {
		...
		if (renderer.backend.isWebGPUBackend === true && renderer.hasFeature("rg11b10ufloat-renderable") === false) console.error("THREE.SSGINode: The device does not support the \"rg11b10ufloat-renderable\" feature which is required for SSGI.");
		const uvNode = uv();
		...
		const spatialOffsets = Fn(([position]) => {
			return float(.25).mul(sub(position.y, position.x).bitAnd(3));
		}).setLayout({
			name: "spatialOffsets",
			type: "float",
			inputs: [{
		...
		const GTAOFastAcos = Fn(([value]) => {
			const outVal = abs(value).mul(float(-.156583)).add(HALF_PI);
			outVal.mulAssign(sqrt(abs(value).oneMinus()));
			return vec2(value.x.greaterThanEqual(0).select(outVal.x, PI.sub(outVal.x)), value.y.greaterThanEqual(0).select(outVal.y, PI.sub(outVal.y)));
		}).setLayout({
			name: "GTAOFastAcos",
			type: "vec2",
			inputs: [{
		...
		const horizonSampling = Fn(([directionIsRight, stepRadius, radiusVS, viewPosition, slideDirTexelSize, initialRayStep, uvNode, viewDir, viewNormal, n]) => {
			...
			Loop({
				...
				let currentOccludedBitfield = angleHorizonInt.greaterThan(uint(0)).select(uint(shiftRight(uint(4294967295), uint(32).sub(MAX_RAY).add(MAX_RAY.sub(angleHorizonInt)))), uint(0)).toConst().shiftLeft(startHorizonInt);
		...
		const gi = Fn(() => {
			...
			const noiseJitterIdx = this._temporalDirection.mul(.02);
			...
			Loop({
				...
				const cos_n = clamp(dot(normalize(projectedNormal).toConst(), viewDir), -1, 1).toConst();
...
* @returns {SSGINode}
*/
var ssgi = (beautyNode, depthNode, normalNode, camera) => new SSGINode(convertToTexture(beautyNode), depthNode, normalNode, camera);
//#endregion
export { SSGINode as default, ssgi };
