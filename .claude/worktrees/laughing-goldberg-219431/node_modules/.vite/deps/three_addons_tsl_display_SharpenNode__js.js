import { Hl as RenderTarget, Ht as NodeMaterial, Yt as NodeUpdateType, ar as TempNode, dd as Vector2, jn as RendererUtils, po as HalfFloatType, yn as QuadMesh } from "./three.webgpu-CajSZNVS.js";
import { Fn, abs, convertToTexture, exp2, float, floor, int, ivec2, max, min, nodeObject, passTexture, textureLoad, textureSize, uv, vec3, vec4 } from "./three_tsl.js";
//#region node_modules/three/examples/jsm/tsl/display/SharpenNode.js
...
*/
var SharpenNode = class extends TempNode {
	static get type() {
		return "SharpenNode";
	}
	/**
	...
	*/
	constructor(textureNode, sharpness = .2, denoise = false) {
		super("vec4");
		/**
	...
	*/
	setup(builder) {
		...
		const rcas = Fn(() => {
			...
			const luma = (s) => s.g.add(s.b.add(s.r).mul(.5));
			const bL = luma(b);
			const dL = luma(d);
			const eL = luma(e);
			const fL = luma(f);
			...
			const RCAS_LIMIT = float(.25 - 1 / 16).toConst();
			const hitMin = min(mn4, e.rgb).div(mx4.mul(4)).toConst();
			const hitMax = vec3(1).sub(max(mx4, e.rgb)).div(mn4.mul(4).sub(4)).toConst();
			const lobeRGB = max(hitMin.negate(), hitMax).toConst();
			const lobe = max(RCAS_LIMIT.negate(), min(max(lobeRGB.r, max(lobeRGB.g, lobeRGB.b)), float(0))).mul(con).toConst();
			const nz = bL.add(dL).add(fL).add(hL).mul(.25).sub(eL).toConst();
			const nzRange = max(max(bL, dL), max(eL, max(fL, hL))).sub(min(min(bL, dL), min(eL, min(fL, hL)))).toConst();
			const nzFactor = float(1).sub(abs(nz).div(max(nzRange, float(1 / 65536))).saturate().mul(.5)).toConst();
			const effectiveLobe = this.denoise.equal(true).select(lobe.mul(nzFactor), lobe).toConst();
			return vec4(b.rgb.add(d.rgb).add(f.rgb).add(h.rgb).mul(effectiveLobe).add(e.rgb).div(effectiveLobe.mul(4).add(1)).toConst(), e.a);
		});
...
* @returns {SharpenNode}
*/
var sharpen = (node, sharpness, denoise) => new SharpenNode(convertToTexture(node), sharpness, denoise);
//#endregion
export { SharpenNode as default, sharpen };
