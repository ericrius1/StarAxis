...
/** Raster-only haze lift on the horizon mesas, driven by the sky's daylight. */
const mesaHaze = uniform(0.22);
...
function createHorizonMesas(): Group {
  ...
  // The emissive lift is a raster-only stand-in for atmospheric haze, and haze
  // only glows while there is daylight in it — held at its full value the
  // horizon mesas out-shine the night sky they are supposed to sit behind.
  material.emissiveNode = color('#514941').mul(mesaHaze);
  // The path tracer computes real aerial perspective, so it must trace these
  // as plain unlit rock or the horizon glows from the inside.
...
export interface TerrainRig {
  group: Group;
  /**
   * Track the sky's daylight so the horizon mesas' haze lift fades out after
   * dusk. `daylight` is 0 at full night, 1 in full daylight.
   */
  setDaylight(daylight: number): void;
}

export function createTerrain(desertMaterial: MeshStandardNodeMaterial): TerrainRig {
  const group = new Group();
  group.name = 'terrain';
  ...
  return {
    ...
    setDaylight: (daylight) => {
      mesaHaze.value = 0.025 + 0.195 * Math.min(1, Math.max(0, daylight));
    },
  };
}
