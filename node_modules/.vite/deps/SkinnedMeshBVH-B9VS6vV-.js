import { Ac as Plane, Il as Ray, Wu as Triangle, bi as BufferAttribute, dd as Vector2, es as Line3, fd as Vector3, gi as Box3, ks as Matrix4 } from "./three.webgpu-CajSZNVS.js";
...
var IS_GT_REVISION_169$1 = parseInt("185") >= 169;
var IS_LT_REVISION_161$1 = parseInt("185") <= 161;
...
var MeshBVH = class MeshBVH extends GeometryBVH {
	...
	*/
	raycastObject3D(object, raycaster, intersects = []) {
		...
		_direction.copy(_ray$1.direction).multiply(_worldScale);
		const scaleFactor = _direction.length();
		const near = raycaster.near / scaleFactor;
		const far = raycaster.far / scaleFactor;
		if (raycaster.firstHitOnly === true) {
			let hit = this.raycastFirst(_ray$1, material, near, far);
			hit = convertRaycastIntersect(hit, object, raycaster);
			if (hit) intersects.push(hit);
		} else {
			const hits = this.raycast(_ray$1, material, near, far);
...
var IS_LT_REVISION_161 = parseInt("185") <= 161;
	...
	raycastObject3D(object, raycaster, intersects = []) {
		...
		this.shapecast({
			...
			intersectsBounds: (box) => {
				if (firstHitOnly) {
					if (!_ray.intersectBox(box, _vec)) return 0;
					let dist;
					if (box.containsPoint(_ray.origin)) dist = 0;
					else {
						_vec.applyMatrix4(matrixWorld);
						dist = raycaster.ray.origin.distanceTo(_vec);
					}
					return dist < closestDistance ? 1 : 0;
				} else return _ray.intersectsBox(box) ? 1 : 0;
			},
			intersectsTriangle: (tri, triIndex) => {
				let point = null;
...
//#endregion
export { SAH as A, CENTER as C, LEAFNODE_MASK_32 as D, IS_LEAFNODE_FLAG as E, NOT_INTERSECTED as O, AVERAGE as S, INTERSECTED as T, LEFT_NODE as _, ExtendedTriangle as a, SPLIT_AXIS as b, getIndexArray as c, BVH as d, BVHTraversalHelper as f, IS_LEAF as g, COUNT as h, OrientedBox as i, SKIP_GENERATION as j, PRIMITIVE_INTERSECT_COST as k, getVertexCount as l, BOUNDING_DATA_INDEX as m, MeshBVH as n, GeometryBVH as o, PrimitivePool as p, getTriangleHitPointInfo as r, generateIndirectBuffer as s, SkinnedMeshBVH as t, isSharedArrayBufferSupported as u, OFFSET as v, CONTAINED as w, arrayToBox as x, RIGHT_NODE as y };
