import { Hl as RenderTarget, Ht as NodeMaterial, Yt as NodeUpdateType, ar as TempNode, dd as Vector2, fd as Vector3, jn as RendererUtils, po as HalfFloatType, yn as QuadMesh } from "./three.webgpu-CajSZNVS.js";
import { Fn, Loop, add, float, int, luminance, mix, nodeObject, passTexture, smoothstep, texture, uniform, uniformArray, uv, vec4 } from "./three_tsl.js";
//#region node_modules/three/examples/jsm/tsl/display/BloomNode.js
...
var _rendererState;
var luminosityHighPass = Fn(({ input, threshold, smoothWidth }) => {
...
var BloomNode = class extends TempNode {
	...
	*/
	setup(builder) {
		...
		this._highPassFilterMaterial.fragmentNode = this.highPassFn({
		...
		for (let i = 0; i < this._nMips; i++) this._separableBlurMaterials.push(this._getSeparableBlurMaterial(builder, kernelSizeArray[i]));
		const bloomFactors = uniformArray([
			1,
			.8,
			.6,
			.4,
		...
		const lerpBloomFactor = Fn(([factor, radius]) => {
			return mix(factor, float(1.2).sub(factor), radius);
		}).setLayout({
	...
	*/
	dispose() {
		for (let i = 0; i < this._renderTargetsHorizontal.length; i++) this._renderTargetsHorizontal[i].dispose();
		for (let i = 0; i < this._renderTargetsVertical.length; i++) this._renderTargetsVertical[i].dispose();
		...
		for (let i = 0; i < this._separableBlurMaterials.length; i++) this._separableBlurMaterials[i].dispose();
	}
	/**
	...
	*/
	_getSeparableBlurMaterial(builder, kernelRadius) {
		...
		const direction = uniform(new Vector2(.5, .5));
		const uvNode = uv();
...
* @returns {BloomNode}
*/
var bloom = (node, strength, radius, threshold) => new BloomNode(nodeObject(node), strength, radius, threshold);
//#endregion
export { bloom, BloomNode as default };
